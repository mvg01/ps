...
int init(int l, int r, int idx) {
    if (l == r) return s[idx] = l;
    int mid = (l + r) / 2;
    int leftidx = init(l, mid, idx * 2);
    int rightidx = init(mid + 1, r, idx * 2 + 1);
    if (v[leftidx] > v[rightidx]) return s[idx] = rightidx;
    else return s[idx] = leftidx;
}

int update(int index, int value, int l, int r, int idx) {
    if (l > index || r < index) return s[idx];
    if (index == l && l == r) {
        v[index] = value;
        return s[idx] = index;
    }
...
int query(int l, int r, int il, int ir, int idx) {
    if (l > ir || r < il) return 100004;
    if (l <= il && r >= ir) return s[idx];
    int mid = (il + ir) / 2;
    int leftidx = query(l, r, il, mid, idx * 2);
    int rightidx = query(l, r, mid + 1, ir, idx * 2 + 1);
    if (v[leftidx] > v[rightidx]) return rightidx;
    else if (v[leftidx] > v[rightidx]) return leftidx;
    else {
